...
X_large = np.zeros((600, 93))
y_large = np.zeros(600)

for i in range(0, 300):
    X_large[i] = X[i]
    X_large[300 + i] = X_noise[i]
    y_large[i] = y[i]
    y_large[300 + i] = y[i]


X_train, X_test, y_train, y_test = train_test_split(X_large, y_large, test_size=0.9, random_state=5)
